...
def atualizar():
    ...
    if ativo in dados_reais:
        ...
        if agora >= proximo_snapshot:
            ...
            log_performance.insert(0, {"horario": agora.strftime("%H:%M"), "ativo": ativo, "preco": preco, "tendencia": m["tendencia"], "rsi": f"{m['rsi']:.1f}"})
...
def atualizar_fin():
    ...
    data = request.json
    financeiro.update({"resultado_dia": data.get('resultado_dia'), "em_aberto": data.get('em_aberto'), "saldo_atual": data.get('saldo_atual'), "conta": data.get('conta')})
...
def get_signal():
    ...
    m = calcular_metricas(ativo)
    return jsonify({"preco": dados_reais[ativo]["preco"], "rsi": float(f"{m['rsi']:.1f}"), "tendencia": m['tendencia'], "fin": financeiro, "posicoes": posicoes_abertas, "logs": log_performance[:5]})
...
def chat():
    ...
    prompt = f"Trader: {user_msg}. Contexto: WIN={dados_reais['WIN']['preco']} (RSI:{m_win['rsi']:.1f}), WDO={dados_reais['WDO']['preco']} (RSI:{m_wdo['rsi']:.1f}). Financeiro: Saldo {financeiro['saldo_atual']}. Curto e técnico."
    try:
